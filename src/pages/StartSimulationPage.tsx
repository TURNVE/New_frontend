...
import { ArrowRight, Briefcase, Crown, GraduationCap, Lock, PlayCircle, Rocket, ShieldCheck, Sparkles, Target, Users } from 'lucide-react';
...
import { FINTECH_PM_SIMULATIONS } from '../features/sim-pm-fintech-track/configs';

const StartSimulationPage = () => {
  usePageSetup();
  const navigate = useNavigate();
  const [showSimulations] = useState(true);

  const fintechSimulations = FINTECH_PM_SIMULATIONS.map((simulation, index) => ({
    id: simulation.id,
    route: `/simulation/${simulation.id}`,
    name: simulation.name,
    companyName: simulation.companyName,
    difficulty: simulation.difficulty,
    challengeDetails: simulation.challengeDetails,
    ...
    access: 'Unlocked',
    summary: simulation.challengeDetails,
  }));
  ...
  const productManagementSimulations = [
    ...fintechSimulations,
    {
      ...
      route: '/simulation/sim-intern-001',
      ...
      weeks: INTERN_ONBOARDING_CONFIG.totalWeeks,
      budget: INTERN_ONBOARDING_CONFIG.budget || 0,
      isUnlocked: true,
      isRecommended: false,
      access: 'Unlocked',
      summary: 'Optional onboarding path for learners who want a lighter workplace introduction before the fintech projects.'
    },
    ...[
  ...
  const startRandomUnlockedSimulation = () => {
    const recommendedSimulation = productManagementSimulations.find((sim) => sim.isRecommended);
    if (recommendedSimulation) {
      startSimulation(recommendedSimulation);
      return;
    }

    const unlockedSimulations = productManagementSimulations.filter((sim) => sim.isUnlocked);
    const availableSimulations = unlockedSimulations.length > 0 ? unlockedSimulations : productManagementSimulations;
    const randomSimulation = availableSimulations[Math.floor(Math.random() * availableSimulations.length)];

    if (randomSimulation) {
      startSimulation(randomSimulation);
    }
  };
        ...
        <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
              Start with PayLoop's fintech activation project. Review messy workplace documents, analyze complaints, prioritize an MVP feature, write a mini PRD, create user stories, and package the work as a portfolio case study.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <button
                onClick={startRandomUnlockedSimulation}
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-border bg-card shadow-2xl shadow-primary/10">
            <img
              src="/images/pm-fintech-workspace.svg"
              alt="Fintech product management simulation workspace"
            ...
            <div className="absolute inset-x-5 bottom-5 rounded-2xl border border-white/40 bg-white/90 p-5 shadow-xl backdrop-blur dark:border-white/10 dark:bg-[#111318]/90">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-foreground">Simulation 1: improve first transaction completion</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                  Start with the full fintech PM project library. Each project follows the same PM internship workflow and produces portfolio-ready artifacts.
                </p>
              </div>
              ...
              {productManagementSimulations.map((sim) => (
                <button
                  ...
                  className={`group flex min-h-[220px] flex-col rounded-2xl border p-5 text-left transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${
                    sim.isUnlocked
                      ? 'border-primary/50 bg-primary/10 shadow-lg shadow-primary/10 ring-1 ring-primary/20 hover:border-primary/70'
                      : 'border-border bg-background/40 opacity-55 grayscale-[0.25] hover:opacity-75 hover:border-amber-400/30'
                  }`}
                >
                  <div className="mb-4 flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        >
                        {sim.isUnlocked ? <GraduationCap className="h-5 w-5" /> : <Briefcase className="h-5 w-5" />}
                      </div>
                      <div>
                    ...
                    {sim.isUnlocked ? (
                      <span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-bold uppercase tracking-widest ${getDifficultyColor(sim.difficulty)}`}>
                        {sim.isRecommended ? 'Start here' : sim.difficulty}
                      </span>
                    ) : (
                      <span
                        className="shrink-0 rounded-full border border-amber-300/30 bg-amber-400/10 p-2 text-amber-600 dark:text-amber-300"
                        aria-label="Premium simulation"
                  ...
                  <div className="mt-auto flex items-center justify-between border-t border-border/60 pt-4">
                    <span className="text-xs font-semibold text-muted-foreground">
                      {sim.isUnlocked ? `${sim.weeks} module workplace project` : `${sim.weeks} week premium project`}
                    </span>
                    <span className={`inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest ${sim.isUnlocked ? 'text-primary' : 'text-amber-600 dark:text-amber-300'}`}>
                      {sim.isUnlocked ? <Rocket className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
                      {sim.isUnlocked ? 'Start' : 'Upgrade'}
                    </span>
                  </div>
                </button>
              ))}
