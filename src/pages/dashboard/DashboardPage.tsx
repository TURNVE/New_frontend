...
export function DashboardPage() {
  ...
  useEffect(() => {
    async function loadData() {
      const storedName = localStorage.getItem('turnve_user_name');
      ...
      setOngoingSimulations(mockSimulations);

      const mockActivities: Activity[] = [
        { id: '1', title: 'Simulation Completed', desc: 'Finished with a score of 85%', time: '2 days ago', icon: 'briefcase' },
        { id: '2', title: 'Simulation Updated', desc: 'Progressed to week 4', time: '5 days ago', icon: 'trend' },
      ];
  ...
  const { displayedText: welcomeText, isTyping } = useTypingEffect(welcomeTitle, {
    speed: 35,
    delay: 200,
    enabled: true,
  });
  ...
  return (
    <div className="animate-fade-in">
      {/* Welcome Section */}
      {isFirstTimeUser ? (
        <ScrollReveal className="mb-6 sm:mb-8 lg:mb-10">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold tracking-tight text-foreground mb-2 sm:mb-3 min-h-[2.5rem]">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-indigo-600">
              {welcomeText}
            </span>
            {isTyping && (
              <span className="inline-block w-0.5 h-[0.9em] ml-0.5 bg-primary animate-pulse align-middle" />
            )}
          </h1>
        ...
        <ScrollReveal className="mb-6 sm:mb-8 lg:mb-10">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold tracking-tight text-foreground min-h-[2.5rem]">
              {welcomeText}
              {isTyping && (
                <span className="inline-block w-0.5 h-[0.9em] ml-0.5 bg-foreground animate-pulse align-middle" />
              )}
            </h1>
          </div>
      ...
      {/* First Time User CTA */}
      {isFirstTimeUser && (
        <ScrollReveal className="relative overflow-hidden bg-gradient-to-br from-[#5e6ad2] via-[#6366d3] to-[#7170ff] rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-5 mb-4 sm:mb-6 lg:mb-8 text-white shadow-lg shadow-[rgba(94,106,210,0.2)] border border-white/10">
      ...
      {/* Quick Actions Grid */}
      <ScrollReveal className="mb-6 sm:mb-8 lg:mb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          <ScrollReveal as="div" delay={0.02}>
            <Link
            to="/simulation/pm-01"
            className={`flex flex-col items-center p-4 sm:p-5 lg:p-6 rounded-2xl transition-all duration-300 tap-target border group ${
              isFirstTimeUser
                ? 'bg-primary/5 border-primary/20 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-primary/40'
                : 'bg-card border-border shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-lg hover:-translate-y-1 hover:border-primary/30'
            }`}
          >
            <div className={`h-12 w-12 sm:h-14 sm:w-14 rounded-xl sm:rounded-[16px] flex items-center justify-center mb-3 sm:mb-4 transition-colors duration-300 ${
              isFirstTimeUser
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div>
          {isFirstTimeUser ? (
            <ScrollReveal className="bg-card rounded-2xl sm:rounded-3xl border border-border shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden">
              ...
              <div className="p-4 sm:p-6">
                <Link to="/simulation/pm-01" className="block group p-4 sm:p-6 rounded-2xl bg-gradient-to-r from-[rgba(94,106,210,0.1)] to-[rgba(113,112,255,0.1)] border border-[rgba(94,106,210,0.2)] hover:shadow-md transition-all duration-300 tap-target relative overflow-hidden">
                  <div className="absolute right-0 top-0 w-24 h-24 sm:w-32 sm:h-32 bg-[#5e6ad2]/10 rounded-full blur-3xl group-hover:bg-[#5e6ad2]/20 transition-colors" />
                  <div className="relative z-10">
            ...
            <ScrollReveal className="bg-card rounded-2xl sm:rounded-3xl border border-border shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
              ...
              <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                {ongoingSimulations.slice(0, 2).map((sim) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      <span className={`text-[10px] sm:text-xs font-semibold uppercase tracking-wide px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md ${
                        sim.status === 'active'
                          ? 'bg-[rgba(94,106,210,0.2)] text-[#7170ff]'
                          : sim.status === 'in-progress'
                          ? 'bg-[rgba(39,166,68,0.2)] text-[#27a644]'
            ...
            <ScrollReveal className="bg-card rounded-2xl sm:rounded-3xl border border-border shadow-[0_4px_20px_rgb(0,0,0,0.03)] p-6 sm:p-10 lg:p-12 text-center">
