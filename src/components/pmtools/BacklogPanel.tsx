...
export const BacklogPanel: React.FC<BacklogPanelProps> = ({ gameState }) => {
  const [items, setItems] = useState<BacklogItem[]>(DEFAULT_BACKLOG_ITEMS);
    ...
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      ...
      <div className="p-4 border-b border-white/5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm flex items-center gap-2">
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-white placeholder-[#a1a1aa]"
            />
          </div>
          <select
      ...
      <div className="flex-1 overflow-x-auto p-4">
        <div className="flex gap-3 h-full min-w-max">
          {columns.map(column => (
            <div
              key={column.id}
              className="w-64 flex-shrink-0 flex flex-col bg-white/[0.02] rounded-xl border border-white/5"
            ...
            >
              ...
              <div className="p-3 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#a1a1aa] bg-white/5 px-1.5 py-0.5 rounded">
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-2">
                {getItemsByStatus(column.id).map(item => (
                  <div
                    ...
                    className={`bg-[#141414] rounded-lg p-3 border border-white/10 hover:border-white/20 cursor-grab active:cursor-grabbing transition-all ${
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${PRIORITY_COLORS[item.priority]} text-white`}>
                        {item.priority}
                      </span>
                    </div>
